...
//	power.pow_motor = Calc_RealPower(voltage.vol_Out, current.cur_Out)/1.3;		//Demon1.0的板子用这个
...
void C_SourceManage_Classdef::Check_CapStatus(void)
{
	...
	{
		capObj.chargeStatus = MID;	/* 需要开启升压充电 */
	}	
}
...
{
	if(capObj.chargeStatus == MID)/* 需要开启升压充电 */
		BAT_BOOST = OPEN;
	else
		BAT_BOOST = CLOSE;
	...
	CANx_SendData(&hcan2,BAT_Boost_Board_Msg.ID,msg_send,1); //修改为can通信
